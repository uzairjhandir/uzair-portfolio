...
async function getPortfolio(slug: string): Promise<any | null> {
  try {
    const res = await axios.get(`${API_URL}/portfolios/${slug}`);
...
async function getAllPortfolios(): Promise<any[]> {
  try {
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const study = await getPortfolio(resolvedParams.slug);
  ...
  return {
    ...
    description: study.description?.substring(0, 160) || study.overview?.substring(0, 160) || '',
    alternates: {
    ...
    openGraph: {
      ...
      images: [
        {
          url: study.featured_image || study.heroImage || 'https://placehold.co/1200x630',
          width: 1200,
          height: 630,
          alt: study.title,
        },
    ...
    twitter: {
      ...
      title: `${study.title} | Case Study`,
      description: study.description?.substring(0, 160) || study.overview?.substring(0, 160) || '',
      images: [study.featured_image || study.heroImage || 'https://placehold.co/1200x630'],
    },
  };
}
...
export default async function CaseStudyPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  const caseStudy = await getPortfolio(resolvedParams.slug);
  ...
  const relatedProjects = allProjects.filter(c => (caseStudy.relatedProjects || []).includes(c.slug)).slice(0, 3);
  ...
  const creativeWorkJsonLd = {
    ...
    "description": caseStudy.description || caseStudy.overview || '',
    "image": [
      caseStudy.featured_image ? (caseStudy.featured_image.startsWith('http') ? caseStudy.featured_image : `${siteUrl}${caseStudy.featured_image}`) : 'https://placehold.co/1200x630'
    ],
    "author": [{
        "@type": "Person",
        "name": "Muhammad Uzair",
      ...
      <section className="relative pt-40 pb-20 overflow-hidden border-b border-white/5">
        <div className="absolute inset-0 z-0">
          <div 
            ...
            style={{ backgroundImage: `url(${caseStudy.featured_image || caseStudy.heroImage || 'https://placehold.co/1920x1080'})` }}
          ></div>
          ...
          <FadeIn>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              <span className="px-4 py-1.5 rounded-full bg-accent/10 text-accent text-sm font-bold tracking-wider uppercase border border-accent/20">
                {caseStudy.category_id || caseStudy.category || 'Project'}
              </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-8 border-y border-white/10 mt-12 bg-white/5 rounded-2xl px-8 backdrop-blur-sm">
              <div>
                ...
                <p className="text-white font-semibold text-lg">{caseStudy.client_name || caseStudy.client || 'Client'}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-sm uppercase tracking-wider mb-2 font-medium">Timeline</p>
                <p className="text-white font-semibold text-lg">{caseStudy.duration || 'N/A'}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-sm uppercase tracking-wider mb-2 font-medium">Team</p>
                <p className="text-white font-semibold text-lg">{caseStudy.teamSize || 'N/A'}</p>
              </div>
              <div>
          ...
          <div className="lg:col-span-8 space-y-24">
            ...
            <FadeIn>
              ...
              <p className="text-lg text-muted-foreground leading-relaxed">
                {caseStudy.description || caseStudy.overview || ''}
              </p>
            </FadeIn>
            ...
            <FadeIn>
              ...
              <div className="space-y-8 border-l-2 border-white/10 ml-5 pl-8 relative">
                {(caseStudy.developmentProcess || []).map((process: any, idx: number) => (
            ...
            <FadeIn>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {[
                  { label: "Lighthouse", value: caseStudy.performance?.lighthouse || '99', icon: <Activity size={24} /> },
                  { label: "SEO", value: caseStudy.performance?.seo || '100', icon: <Search size={24} /> },
                  { label: "Accessibility", value: caseStudy.performance?.accessibility || '100', icon: <Award size={24} /> },
                  { label: "Best Practices", value: caseStudy.performance?.bestPractices || '100', icon: <ShieldCheck size={24} /> },
                  { label: "Load Time", value: caseStudy.performance?.loadTime || '0.8s', icon: <Clock size={24} /> },
                  { label: "TTFB", value: caseStudy.performance?.ttfb || '100ms', icon: <Zap size={24} /> },
                ].map((stat, idx) => (
              ...
              {relatedProjects.map(project => (
                <Link key={project.slug} href={`/case-studies/${project.slug}`} className="group block rounded-2xl overflow-hidden bg-white/5 border border-white/10 hover:border-accent/50 transition-colors">
                  <div className="relative h-64 w-full overflow-hidden">
                    <div className="absolute inset-0 bg-cover bg-center transition-transform duration-1000 group-hover:scale-105" style={{ backgroundImage: `url(${project.featured_image || 'https://placehold.co/800x600'})` }}></div>
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                      <span className="text-accent text-xs font-bold tracking-widest uppercase mb-2 block">{project.category_id || 'Project'}</span>
