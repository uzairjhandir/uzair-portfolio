    ...
    <section id="portfolio" className="py-24 relative bg-transparent border-t border-white/5">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          ...
          <FadeIn delay={0.2}>
            <Link href="/portfolio" className="inline-flex items-center space-x-2 text-white hover:text-accent transition-colors font-medium hover-trigger border border-white/10 px-6 py-3 rounded-full hover:bg-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          ) : portfolioData.slice(0, 4).map((project: PortfolioProject, index: number) => (
                  ...
                  <div className="flex items-center justify-between border-t border-white/10 pt-6 mt-auto">
                    ...
                    <div className="flex space-x-3 relative z-20 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] delay-200">
                      {project.repository_url && (
                        <a href={project.repository_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 transition-all text-white hover:scale-110">
                          <svg role="img" viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d={siGithub.path} /></svg>
                        </a>
                      )}
                      {project.project_url && (
                        <a href={project.project_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary hover:bg-accent border border-transparent transition-all text-white hover:scale-110 shadow-[0_0_15px_rgba(var(--primary-rgb),0.5)]">
