...
import { useNavigationQuery } from "@/lib/query/navigation/queries";
import { useSubscribeNewsletterMutation } from "@/lib/query/newsletter/mutations";
...
export function Footer() {
  ...
  const { mutateAsync: subscribe } = useSubscribeNewsletterMutation();
  ...
  const { data: navData, isLoading } = useNavigationQuery('footer');
          ...
          <ul className="space-y-4 text-sm text-muted-foreground">
            {isLoading ? (
              <div className="space-y-4 animate-pulse">
                {[1,2,3,4,5].map(i => <div key={i} className="h-4 w-24 bg-white/10 rounded" />)}
              </div>
            ) : footerLinks.length > 0 ? (
              footerLinks.map((item) => (
                <li key={item.id || item.label}>
                  <Link href={item.url} className="group flex items-center hover:text-accent transition-colors hover-trigger w-fit">
              ...
              // Fallback if footer menu is empty
        ...
        <div>
          ...
          <form className="relative group" onSubmit={handleSubscribe}>
            <input 
              ...
              required
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-accent transition-colors disabled:opacity-50"
              disabled={status === "loading" || status === "success"}
            />
            <button 
              type="submit" 
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-white/10 rounded-md text-white hover:bg-accent transition-colors hover-trigger disabled:opacity-50"
              disabled={status === "loading" || status === "success"}
            >
              <ArrowUpRight size={16} />
            </button>
          </form>
          {status === "success" && <p className="text-xs text-green-400 mt-2">Subscribed successfully!</p>}
          {status === "error" && <p className="text-xs text-red-400 mt-2">Failed to subscribe. Please try again.</p>}
        </div>
