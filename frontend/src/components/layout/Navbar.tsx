...
export function Navbar() {
  ...
  const { data: navData, isLoading } = useNavigationQuery('header');
  const navLinks = navData?.items || defaultNavLinks;
          ...
          <nav className="hidden md:flex items-center space-x-2">
            {isLoading ? (
              <div className="flex space-x-4 animate-pulse">
                {[1,2,3,4,5].map(i => <div key={i} className="h-4 w-16 bg-white/10 rounded-full" />)}
              </div>
            ) : navLinks.map((link) => {
              const isActive = pathname === link.url || (pathname === '/' && link.url.startsWith('/#') && link.url !== '/');
