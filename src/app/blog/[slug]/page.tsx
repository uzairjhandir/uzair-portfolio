...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  const postIndex = blogData.findIndex((p) => p.slug === resolvedParams.slug);
        ...
        <div className="container mx-auto px-6 relative z-10 max-w-5xl">
          <FadeIn>
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-muted-foreground font-medium border-t border-white/10 pt-8 mt-8">
              ...
              <span className="flex items-center gap-2"><Calendar size={16} className="text-white/40" /> {post.date}</span>
              <span className="flex items-center gap-2"><Clock size={16} className="text-white/40" /> {post.readingTime}</span>
            </div>
          </FadeIn>
        </div>
      </section>
          ...
          <div className="lg:col-span-8">
            <FadeIn>
              ...
              <div className="mt-16 pt-8 border-t border-white/10 flex flex-wrap gap-3">
                {post.tags.map(tag => (
                  <span key={tag} className="flex items-center gap-1.5 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white font-medium hover:bg-white/10 transition-colors cursor-pointer">
                    <Hash size={14} className="text-accent" /> {tag}
                  </span>
                ))}
              </div>
                ...
                <div className="p-8 rounded-3xl bg-secondary/30 border border-white/10 backdrop-blur-md">
                  ...
                  <ul className="space-y-4">
                    {post.toc.map((item, idx) => (
                      <li key={idx}>
                        <a href={`#${item.id}`} className="text-muted-foreground hover:text-accent transition-colors block text-sm">
                          {item.title}
                        </a>
                      </li>
                    ))}
              ...
              {relatedPosts.map((relatedPost) => (
                <Link key={relatedPost.slug} href={`/blog/${relatedPost.slug}`} className="group flex flex-col bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-white/30 transition-all duration-300">
                  <div className="relative h-48 overflow-hidden">
                    <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url(${relatedPost.heroImage})` }}></div>
                  </div>
                  <div className="p-6">
                    <span className="text-accent text-xs font-bold uppercase tracking-wider mb-2 block">{relatedPost.category}</span>
